<template>
  <div :class="'sidebar ' +  checkVersion()">
    <SidebarHeader/>
    <SidebarForm/>
        ...
        <template v-for="(item, index) in navItems">
          ...
          <template v-else>
            <template v-if="item.children">
              ...
              <SidebarNavDropdown :name="item.name" :url="item.Description.PageLink == 'Empty' ? undefined : item.url" :icon="item.icon">
                <template v-for="(childL1, index) in item.children">
                  <template v-if="childL1.children">
                    <!-- Second level dropdown -->
                    <SidebarNavDropdown :name="childL1.name" :url="childL1.Description.PageLink == 'Empty' ? undefined : childL1.url" :icon="childL1.icon">
